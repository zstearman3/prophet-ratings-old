module ApplicationHelper
  ...
  def sort_link(column, title = nil, init_direction = 'asc', season = current_year, conference_id = nil)
    title ||= column.title
    ...
    if conference_id.to_f > 0
      link_to "#{title} <span class='#{icon}'></span>".html_safe, {column: column, direction: direction, season: season, conference_id: conference_id}
    else
